"""
Example script demonstrating the non-taxonomy discovery workflow with OntoLearner.

This script shows how to:
1. Import an ontology
2. Perform train-test split
3. Run non-taxonomy discovery task with selected LLM and retriever
...
def main():
    """Run the non-taxonomy discovery example workflow"""
    ...
    logger.info(f"Extracted {len(data.type_non_taxonomic_relations.non_taxonomies)} non-taxonomic relations")
    ...
    test_subset = test_data.type_non_taxonomic_relations.non_taxonomies[:10]

    for non_tax_rel in test_subset:
        head = non_tax_rel.head
        tail = non_tax_rel.tail
        ground_truth = non_tax_rel.relation  # The actual relation type

        # Make prediction
        raw_input = (head, tail)
        predicted = rag_learner.predict(raw_input, task="non-taxonomy-discovery")

        # Calculate metrics (simplified for this example)
        # You would need to implement a proper calculate_relation_metrics function
        is_correct = predicted.lower() == ground_truth.lower() or ground_truth.lower() in predicted.lower()
        ...
        logger.info(f"Ground Truth Relation: {ground_truth}")
