...
class TopologyMetrics(BaseModel):
    ...
    density: float = Field(..., description="Ratio of actual connections to possible connections", ge=0.0, le=1.0)
    # Degree-related metrics
    avg_degree: float = Field(..., description="Average number of connections per node", ge=0.0)
    ...
    avg_path_length: float = Field(..., description="Average shortest path length between connected nodes", ge=0.0)
    diameter: int = Field(..., description="Longest shortest path in the graph", ge=0)
