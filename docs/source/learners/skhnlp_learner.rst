...
The :class:`LearnerPipeline` class runs the zero-shot taxonomy learner end-to-end:
it calls the LLM, applies normalization, and evaluates predictions.
...
.. code-block:: python
    ...
    outputs = pipe(
        train_data=train_data,  # zero-shot; largely ignored by the LLM learner
        test_data=test_data,
        task="taxonomy-discovery",
        evaluate=True,
        ontologizer_data=False,
    ...
    # Display total elapsed time for prediction + evaluation
...
The :class:`LearnerPipeline` can also be used with :class:`SKHNLPSequentialFTLearner`
to run supervised taxonomy-discovery experiments end-to-end. It fine-tunes the BERT model,
produces predictions, and computes evaluation metrics in a single pipeline call.
