...
The experiment addressed the **taxonomy discovery** task (Task B of LLMs4OL 2024), which involves extracting the parent class from a child class. This was modeled as a **multi-class classification** problem using the **GeoNames dataset**.

**Models and Approaches:**
The study compared two main approaches:
* **BERT-Based Approach:** Involved **fine-tuning** the **BERT-Large** model as a single binary classifier that was iteratively used for the 9 parent classes. Experiments focused on varying the sets of fine-tuning prompts.
* **LLaMA-Based Approach:** Utilized the **LLaMA 3 70B** model, focusing on designing and modifying different **prompts** based on "Classification" and "Hierarchy (is-a)" concepts. An auxiliary **substring function** was used in evaluation to handle partial answers.
